...
usage() { echo -e "THIS IS AN EDIT! \n Usage in Spyne container: \n docker exec spyne bash $0 -s {path to samplesheet.csv starting with /data/} -r <run_id starting with /data> -e <experiment_type> <OPTIONAL: -p amplicon_library> <optional: -c> \n
Usage in git cloned CLI: \n bash $0 -s {path to samplesheet.csv } -r <run_id > -e <experiment_type> <OPTIONAL: -p amplicon_library> <optional: -c> <optional: -n > " 1>&2; exit 1;}
...
if [ -f "${RUNPATH}/report.html" ] && [ -n "${CLEANUP}" ]; then
