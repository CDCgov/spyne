...
# check args
try:
    data_root, run = "/".join(argv[1].split("/")[:-1]), argv[1].split("/")[-1]
    print(f"data_root={data_root}\nrun={run}")
except:
    exit(f"\n\tUSAGE:\n\t\tpython3 {__file__} <full/path/to/mira-run-dir>\n\n")
...
# image locations
mira_logo = (
    f"{dirname(realpath(__file__))}/assets/mira-logo-midjourney_20230526_rmbkgnd.png"
)
favicon = f"{dirname(realpath(__file__))}/assets/favicon.ico"
excel_logo = f"{dirname(realpath(__file__))}/assets/Microsoft_Excel-Logo.png"
...
try:
    bdp_html = pio.read_json(
        f"{data_root}/{run}/dash-json/barcode_distribution.json"
    ).to_html(
        config={
...
try:
    chm_html = pio.read_json(f"{data_root}/{run}/dash-json/heatmap.json").to_html(
        config={
            "toImageButtonOptions": {
                "format": "svg",
...
try:
    dais_var_pd = pd.read_json(
        f"{data_root}/{run}/dash-json/dais_vars.json", orient="split"
    )
...
try:
    mvdf = pd.read_json(f'{data_root}/{run}/dash-json/alleles.json', orient="split")
...
try:
    indels_df = pd.read_json(f'{data_root}/{run}/dash-json/indels.json', orient="split")
...
# link to fastas
fasta_links_html = '''<h2>Fasta downloads</h2><p3>(Right-click->"Save link as...")</p3><br><p>'''
try:
    for f in glob.glob(f'{data_root}/{run}/*fasta'):
        kind = f.split('/')[-1]
